...
void solve()
{
    ...
    {       
        ...
        cardPoints.push_back(counter);

    }

    vector<int> numberOfCards(cardCounter, 1);

    for (int i = 0; i < cardCounter; i++)
    {
        for (int k = numberOfCards[i]; k > 0; k--)
        {
            int actCardNumber = cardPoints[i];

            int internalCounter = 0;
            for (int j = actCardNumber; j > 0; j--)
            {
                internalCounter++;
                numberOfCards[i+internalCounter]++;
            }
        }
    }
    ...
    cout << "\nResult : ";
    cout << accumulate(numberOfCards.begin(), numberOfCards.end(), 0) << "\n\n";
